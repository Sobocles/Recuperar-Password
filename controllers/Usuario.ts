...
export default class Usuarios {
    ...
    public async crearUsuario(req: Request, res: Response) {
        ...
        try {
            ...
            if (!dbUsuario.imagen) { //foto por defecto que tendra el usuario
    ...
    public async recuperarPassword(req: Request, res: Response) {
        ...
        try {
            ...
            // Crear contraseña nueva, esta contraseña sea reemplazada en la nueva cuenta, en la cuenta que estamos intentando recuperar
            const passwordNew = generate({ //genera contraseña aleatoria
                length: 6, //length: Especifica la longitud de la contraseña, en este caso se ha establecido en 6 caracteres.
                numbers: true, // Indica si se deben incluir números en la contraseña, en este caso se ha establecido como true, lo que significa que se incluirán números en la contraseña generada.
                uppercase: false //Indica si se deben incluir letras mayúsculas en la contraseña, en este caso se ha establecido como false, lo que significa que no se incluirán letras mayúsculas en la contraseña generada.
            });
            ...
            const salt = bycript.genSaltSync();
            //se crea un objeto iuser de tipo IUsuariocon y se añade  la contraseña generada encriptandola (la nueva contraseña)
            ...
            const dbUsuario = await Usuario.updateOne({ _id }, { //se utiliza el método updateOne() del modelo Usuario de Mongoose para actualizar los datos del usuario en la base de datos. El primer argumento del método updateOne() es una condición de búsqueda para identificar el usuario que se desea actualizar. En este caso, se utiliza el _id para buscar el usuario específico. El segundo argumento del método updateOne() es un objeto que contiene las actualizaciones que se desean realizar. En este caso, se utiliza el operador $set para especificar los campos y los nuevos valores que se desean actualizar. El objeto iuser se pasa como valor para $set, lo que significa que se actualizarán las propiedades nombre, apellido, email, usuario y password del usuario encontrado.
                $set: iuser
            });
